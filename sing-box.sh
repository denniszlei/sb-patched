...
# 当前脚本版本号
VERSION='v1.3.10 (2026.04.25)-patched'
...
http2://$(echo -n "${UUID[22]}:${UUID[22]}@${SERVER_IP_2}:${PORT_NAIVE}" | base64 -w0)?peer=${TLS_SERVER}&padding=1&uot=2&hpkp=${SELF_SIGNED_200_FINGERPRINT_SHA256}#${NODE_NAME[22]// /%20}%20${NODE_TAG[11]}%20http2
...
bash <(wget --no-check-certificate -qO- ${SCRIPT_SOURCE}) \$@
EOF
